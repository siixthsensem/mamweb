(function(){
  ...
  window.loginWithPassword=async event=>{event.preventDefault();const form=event.currentTarget,db=await client();status('กำลังเข้าสู่ระบบ...');const {data,error}=await db.auth.signInWithPassword({email:form.elements.email.value.trim(),password:form.elements.password.value});if(error){status(error.message==='Invalid login credentials'?'อีเมลหรือรหัสผ่านไม่ถูกต้อง':error.message);return}await redirectUser(data.user)};
  window.registerUser=async event=>{event.preventDefault();const form=event.currentTarget,db=await client();status('กำลังสร้างบัญชี...');const {data,error}=await db.auth.signUp({email:form.elements.email.value.trim(),password:form.elements.password.value,options:{data:{name:form.elements.name.value.trim()},emailRedirectTo:'https://siixthsensem.github.io/mamweb/login.html'}});if(error){status(error.message);return}if(!data.session){status('กรุณาตรวจอีเมลเพื่อยืนยันบัญชีก่อนเข้าสู่ระบบ')}else await redirectUser(data.user)};
